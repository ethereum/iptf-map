...
works-best-when:
  ...
  - Moderate set sizes (up to ~100k elements)
  - The function to compute is known at circuit-compile time
avoid-when:
  - Only the intersection itself is needed (DH or OPRF variants are simpler and faster)
  - Sets exceed 100k elements (circuit size becomes prohibitive)
  - The function over the intersection changes frequently (recompilation overhead)
dependencies: [Garbled circuits (Yao/GMW), OT extensions, Boolean circuit compiler]
context: both
crops_profile:
  cr: high
  os: yes
...
Two parties each hold a private set and want to compute a function over their shared elements without exposing non-matching entries. This variant encodes intersection logic and the desired aggregate into a Boolean circuit. Using garbled circuits (Yao's protocol) or secret-shared circuits (GMW), both parties evaluate the circuit jointly. More expensive per element than DH-based PSI, but able to compute arbitrary functions over the intersection: cardinality, sum, threshold checks, or filtered subsets.

## Ingredients

- **Cryptography**: Garbled circuits (Yao) or secret-shared circuits (GMW), Oblivious Transfer (OT) extensions, sort-compare-shuffle circuit design
...
2. **Compile**: Both parties compile a Boolean circuit C that encodes sort-compare-shuffle PSI plus the aggregate function F. The circuit takes both sets as input and outputs F(intersection).
3. **Garble** (Yao variant): A, acting as garbler, generates garbled tables for every gate in C and sends the garbled circuit to B.
...
- **Soundness**: Malicious security achievable via cut-and-choose or dual execution.
...
- Requires a fixed function at compile time. Changing the aggregate function means recompiling and re-garbling the circuit.
- Semi-honest security by default. Malicious security via authenticated garbling (Wang-Ranellucci-Katz 2017) adds ~2-3x overhead over semi-honest.
- Asymmetric roles in Yao's protocol (garbler vs evaluator). Both parties learn the output, but the garbler must transmit the full garbled circuit. GMW avoids this asymmetry at the cost of more communication rounds.
