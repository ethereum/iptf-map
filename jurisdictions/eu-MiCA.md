---
title: "Jurisdiction: EU / MiCA"
status: ready
region: EU
scope:
  entities: [CASPs, token issuers (ART/EMT), custodians, trading venues]
  activities: [issuance, custody, trading, stablecoins, staking]
key-regulations:
  - MiCA (Reg. (EU) 2023/1114)
  - Transfer of Funds Regulation ("Travel Rule")
  - DORA (Reg. (EU) 2022/2554)
last_reviewed: 2026-06-19
---

> Developer orientation. Not legal advice.

## At a Glance

The EU offers a single, passportable regime under MiCA: prescriptive but predictable. Stablecoins (asset-referenced and e-money tokens) came into scope mid-2024; CASP authorisation and conduct rules apply from end-2024. The AML "Travel Rule" and DORA operational-resilience obligations sit alongside MiCA for in-scope entities. The regime is permissive but heavily regulated.

## What to Watch

- Classification line between MiCA crypto-assets and MiFID financial instruments.
- ESMA scrutiny of marketing that overstates regulatory coverage.
- DORA resilience expectations for authorised entities.

## See also

- [MiCA Regulation (EU 2023/1114) – EUR-Lex](https://eur-lex.europa.eu/eli/reg/2023/1114/oj)
- [Jurisdiction: EU Data Protection & PETs](./eu-data-protection.md)
